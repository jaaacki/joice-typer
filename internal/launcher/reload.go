...
type runtimeReloadDeps struct {
	...
	newTranscriber      func(context.Context, string, string, string, int, string, string, bool, *slog.Logger) (apppkg.Transcriber, error)
...
func applyReloadedConfig(runtime *runtimeConfigState, app *apppkg.App, newCfg configpkg.Config, logger *slog.Logger, deps runtimeReloadDeps) error {
	...
	transcriberChanged := oldCfg.Language != newCfg.Language ||
		...
		oldCfg.Translate != newCfg.Translate
	...
	if transcriberChanged {
		...
		stagedTranscriber, err = deps.newTranscriber(reloadCtx, modelPath, newCfg.ModelSize, newCfg.Language, newCfg.SampleRate, newCfg.DecodeMode, newCfg.PunctuationMode, newCfg.Translate, logger)
		if err != nil {
