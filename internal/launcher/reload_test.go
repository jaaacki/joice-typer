...
func TestApplyReloadedConfig_RollsBackOnTranscriberFailure(t *testing.T) {
	...
	err := applyReloadedConfig(runtime, app, configpkg.Config{
		...
		newTranscriber: func(ctx context.Context, modelPath, modelSize, language string, sampleRate int, decodeMode, punctuationMode string, translate bool, logger *slog.Logger) (apppkg.Transcriber, error) {
