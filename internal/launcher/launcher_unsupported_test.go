...
func TestRunUnsupportedListDevicesFlag(t *testing.T) {
	...
	if !strings.Contains(stderr.String(), "bootstrap build") {
		t.Fatalf("expected unsupported error, got %q", stderr.String())
	}
}

func TestRunUnsupportedDefaultMentionsBootstrapOnly(t *testing.T) {
	var stdout bytes.Buffer
	var stderr bytes.Buffer
	...
	if !strings.Contains(stderr.String(), "bootstrap") {
		t.Fatalf("expected bootstrap-only message, got %q", stderr.String())
	}
}
